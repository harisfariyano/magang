...
class Profil extends MY_Controller {
	...
	public function update()
	{
		...
		if ($this->input->post('password_sekarang')){
			...
			if (password_verify($passlama, $cek->row('user_password'))){
				if (password_verify($passbaru1, $passbaru2)){
					...
					notifikasi_redirect("danger", "Password baru yang Anda masukkan tidak sama", $_SERVER['HTTP_REFERER']);
				}
			} else {
				notifikasi_redirect("danger", "Password saat ini yang Anda masukkan salah", $_SERVER['HTTP_REFERER']);
			}
		} else {
		...
		if ($update){
			...
			notifikasi_redirect("danger", "Update profil gagal", $_SERVER['HTTP_REFERER']);
		}
	}
